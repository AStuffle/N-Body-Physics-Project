...
%}
Ga=43.410; % GMa in units of AU and years
Gb=35.545;  % GMb 
% Star A
r1=0.9*9.582/2;
m(1)=1.100;
    ...
    for j=1:5 % j is the object being checked
        ...
        for k=1:5
            if j~=k % IF j does not equal k
                ...
                ax(j,i)=Ga*m(k)*(x(k,i)-x(j,i))/r(j,k,i)^3+ax(j,i); % this sums all the x forces of all the objects on object j
                ay(j,i)=Ga*m(k)*(y(k,i)-y(j,i))/r(j,k,i)^3+ay(j,i); % this sums all the y forces of all the objects on object j
            end
        end
